...
\begin{figure}
  ...
  \begin{subfigure}{0.45\textwidth}
    \centering
    \begin{tikzpicture}[every node/.style={draw, circle},node distance=2.5cm]
      \node [label={[label distance=-1.0cm]180:$S$},
             rectangle,
             rounded corners,
             minimum width=4cm,
             minimum height=1.6cm] (S) at (0,0) {};

      \node [rectangle, rounded corners, minimum width=2.3cm]
            (ControlPlane) at (0.6, 0.35) {Control plane};

      \node [rectangle, rounded corners, minimum width=2.3cm]
            (DataPlane) at (0.6, -0.35) {Forwarding plane};
    ...
    \end{tikzpicture}
    \caption{A typical switch ($S$) implements both the control and
             forwarding planes.  Contrast this to fig.~\ref{figure:decoupling.planes}.}
  ...
  \begin{subfigure}{0.45\textwidth}
    ...
    \begin{tikzpicture}[every node/.style={draw, circle},node distance=2cm]
      ...
      \node (ControlPlane) [draw=none, right=0.5cm of T] {Control plane};
      \node (DataPlane) [draw=none, below of=ControlPlane] {Forwarding plane};
      \node [rectangle, rounded corners] (FlowTable) [left of=S] {$F$};

      \draw (T) -- node (TLS) [draw=none, left] {\footnotesize \acs{TLS}} (S);
      \draw [dashed] (S) -- (h1);
      \draw [dashed] (S) -- (h2);
      \draw [dashed] (S) -- (h3);
      \draw [dashed] (S) -- (FlowTable);

    \end{tikzpicture}
    \caption{Decoupling of control and forwarding  planes in software--defined
      networking.  The controller $C$ has been moved out of the switch
        $S$---but not the forwarder $F$.
        When using OpenFlow, $S$ and $C$ communicate over a secure \acs{TLS}
