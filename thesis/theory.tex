...
instance, the \textit{Ngaro virtual machine} can run Forth.} and there is
little overhead in interpreting the code.
...
There is almost no syntax in Forth.  It uses \acc{RPN}, so programs tends to
...
To test this word, one can write \texttt{12 square .}, which pushes 12 on
the stack, replaces the number with its square \texttt{144} and then the dot
will take the number on the top of the stack and print it.

Because of the \acc{RPN}, some bits of code seem to be in reverse.  This is
