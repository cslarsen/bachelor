...
\begin{figure}
  ...
  \begin{tikzpicture}[every node/.style={draw, circle},node distance=2.5cm]
    ...
    \node [rectangle, rounded corners, minimum width=2.3cm]
          (DataPlane) at (0.6, -0.35) {Data plane};
...
\begin{figure}
  ...
  \begin{tikzpicture}[every node/.style={draw, circle},node distance=2cm]
    ...
    \node (ControlPlane) [draw=none, right=2cm of T] {Control plane};
    \node (DataPlane) [draw=none, below of=ControlPlane] {Data (or \textit{forwarding}) plane};
    \node [rectangle, rounded corners] (FlowTable) [left of=S] {$F$};

    \draw (T) -- node (TLS) [draw=none, left] {\footnotesize OpenFlow} (S);
    \draw [dashed] (S) -- (h1);
    \draw [dashed] (S) -- (h2);
    \draw [dashed] (S) -- (h3);
