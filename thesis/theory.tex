...
\begin{figure}
  \centering
  \begin{tikzpicture}[every node/.style={draw, circle}, node distance=1.75cm]
    ...
    \node (ControlPlane) [draw=none, left=2cm of T] {Control plane};
    \node (DataPlane) [draw=none, below of=ControlPlane] {Data plane};
    \node (FlowTable) [left of=S] {$F$};

    \draw (T) -- node [draw=none, right] {OpenFlow over TLS}(S);
    \draw [dashed] (S) -- (h1);
    \draw [dashed] (S) -- (h2);
    \draw [dashed] (S) -- (h3);
    \draw (S) -- (FlowTable);
