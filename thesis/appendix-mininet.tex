...
\textit{Enable Network Adapter} and \textit{Attached To} set to
\textit{NAT}.  Under the advanced options, I have set the \textit{MAC
address} to \texttt{FEEDFACEBEEF} (this may not be important for the
operation of the tests).

\textit{Adapter 2} is attached to \textit{Host-only Adapter} with
\textit{name} set to \textit{vboxnet0}.
...
\todo{Reproduser steg her, så vi slipper å se på link, og slik det funker
  som det skal.}
