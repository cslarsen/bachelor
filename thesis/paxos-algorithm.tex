...
\todo{Sjekk at dette er korrekt! Og ikke glem at vi må ha med en ekstra
  ...
  \todo{Forklar forskjellen på de forskjellige nevnte fasene, 1a, 1b, 2a,
    2b, dette er kjempeviktig for å få en god forståelse.}
...
\todo{crnd er statisk, skriv om det, men skriv at vi har valgt å ta det med
  likevel for å gjøre alt klart for å kjøre full paxos ved en senere
    utvidning.}
  ...
  \begin{algorithmic}
    ...
    \On{$\langle \texttt{ACCEPT}, n, seq, v \rangle$}{$leader$}
      \If{$n \geq rnd$} % \wedge n \neq vrnd$}
        ...
        \State $slot.vval\gets v$ \Comment{The client packet}
        \ForIn{$node$}{$N$}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
    ...
    \On{$\langle \texttt{LEARN}, n, seq \rangle$}{$acceptor$}
      \If{$got\_{}majority(n)$}
        \ForIn{$host$}{$H$}
          \State \SendTo{\textit{client packet}}{$host$}
        \EndForIn
      \EndIf
    \EndOn
  \end{algorithmic}
...
\todo{Fiks simplified algoritmer, de har en del feil og mangler}
\todo{Endre rekkefølge, client først}
