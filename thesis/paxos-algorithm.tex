...
\begin{gather}
  ...
  \label{theorem:crnd_mod_N}
\end{gather}

where $n$ is the node and $N$ is the set of all nodes.  This gives the
definition of our $\textbf{pickNext}$ function in
\ref{algorithm:paxos.simplified.pickNext}.

\begin{algorithm}
  \caption{Definition of \textbf{pickNext} based on equation \ref{theorem:crnd_mod_N}}
  ...
  \begin{algorithmic}
    ...
    \Function{$\textbf{pickNext}$}{}
      \State $\textbf{return}\ crnd + |N|$ \Comment{Unique per equation \ref{theorem:crnd_mod_N}}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
