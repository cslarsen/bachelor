...
matches\index{OpenFlow!matching} and Forth.
...
the IP-protocol\index{Paxos!IP-protocol}.

A nice trick in OpenFlow is just to use Ethernet packets\index{Ethernet}
and identify them by marking the \textit{Ethernet
  type}-field\index{Ethernet!type-field} with a special value.
...
Ethernet-header\index{Ethernet!header} is that it
makes it very easy to match such packets in the flow
tables\index{OpenFlow!flow table}\index{OpenFlow!matching}.  It also
...
we would have to use IP.\todo{Disse MÅ/BØR være i network order endianness,
  det er det de bruker i openflow tror jeg, sjekk og gjør det}
...
The payload\index{payload} is simply a flat vector of unsigned 32-bit integers.  The length
of the vectors are determined by the type of message.  For practical
purposes (being that we are developing on Intel
    x86-CPUs\index{Intel}\index{x86}) we'll use
little-endian\index{endianness!little-endian} numbers, although a better alternative would be to go for
\textit{network order}---which is big-endian\index{endianness!big-endian}.\footnote{Our goal is to
...
The corresponding structure in the programming language C\index{C}, after extracting
the Ethernet type\index{Ethernet!type-field} and parameters, would be
...
As the structure above is defined in the C\index{C} language, the padding will be
implicit by the rules of the C programming language standard and
platform.\footnote{On my OS X\index{OS X} x86\_{}64\index{x86} computer, the size of the structure
is 12 bytes---i.e.,~no padding\index{padding}, as it aligns on a word boundary.}
...
If we intended to implement full Paxos\index{Paxos!message structure}, we
...
changes\index{OpenFlow!link-status}.  Also, a
...
bytecode\index{bytecode}\index{Paxos!bytecode} and forward packets based on the return value from the code.
...
to install flows with these new actions\index{OpenFlow!actions}.  However, because of the scope of
...
One would first have to modify the OpenFlow protocol with new
  actions\index{OpenFlow!extensios},
...
Forth.  The value $|N|$\index{Paxos!round number} needs to be set in an initialization word,
...
\texttt{ACCEPT}-messages\index{Paxos!accept-message} from the Paxos
leader\index{Paxos!leader}:
