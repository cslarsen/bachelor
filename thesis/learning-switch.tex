...
vSwitch, they call this an \textit{upcall}\index{upcall}.}
...
typically one sets the idle timeout\index{idle
  timeout|see{timeout}}\index{timeout!idle} to 10 seconds and the
hard timeout\index{hard timeout|see{timeout}}\index{timeout!hard} to 60
...
port\index{forwarding}.

This algorithm has been implemented in Python\index{Python} using the POX\index{POX} controller, with
...
tutorial\index{OpenFlow!tutorial}.\todo{cite!}
