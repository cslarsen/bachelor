...
%
But adding a second switch will not be sufficient.  Should the hosts on the
two end up in different states---in the case packet loss, for example---the
two would not be able to decide whose state is correct.
%
We will therefore use three switches.
...
%
This causes an \textit{ordering problem}. If packets reach hosts at
different times, they will also arrive in different \textit{order}.
