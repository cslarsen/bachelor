...
\todo{Kopier figurer ovenfra og lag PAXOS--noder med kanter mot switch.}
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[every node/.style={draw},x=0.5cm,y=0.5cm]
    \def\y{-4}
    \node [circle] (S)  at ( 0,  0) {$S_1$};
    \node [rectangle, minimum width=1.8cm, minimum height=2cm] (h1) at (-5, \y) {$h_1$};
    \node [rectangle, minimum width=1.8cm, minimum height=2cm] (h2) at ( 0, \y) {$h_2$};
    \node [rectangle, minimum width=1.8cm, minimum height=2cm] (h3) at ( 5, \y) {$h_3$};

    \draw (S) -- (h1);
    \draw (S) -- (h2);
    \draw (S) -- (h3);

    \node (P1) at (-5, \y-1.1) {Paxos};
    \node (P2) at ( 0, \y-1.1) {Paxos};
    \node (P3) at ( 5, \y-1.1) {Paxos};
  \end{tikzpicture}
  \caption{Support for Paxos on the servers $h_1, \dots, h_3$ requires a copy of the Paxos code on each server.}
  \label{figure:paxos.on.servers}
\end{figure}

Each server needs to have an implementation of Paxos code running on each
instance.   It means the software developer has to specifically add support
for Paxos when designing the server code.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[every node/.style={draw},x=0.5cm,y=0.5cm]
    \def\y{-5}
    \def\x{7}
    \node [rectangle, minimum width=1.8cm, minimum height=2cm] (S1)  at ( 0, 0) {$S_1$};
    \node [rectangle, minimum width=1.8cm, minimum height=2cm] (S2) at ( \x, 0) {$S_2$};
    \node [dashed, rectangle, minimum width=1.8cm, minimum height=2cm] (Sn) at ( \x + \x, 0) {$S_n$};

    \node [circle] (h1) at (-2.5, \y) {$h_1$};
    \node [circle] (h2) at ( 2.5, \y) {$h_2$};
    \node [circle] (h3) at (\x, \y) {$h_3$};
    \node [dashed, circle] (hn) at (\x + \x, \y) {$h_n$};

    \node (P1) at (0, -1.25) {Paxos};
    \node (P2) at (\x, -1.25) {Paxos};
    \node [dashed] (Pn) at (\x + \x, -1.25) {Paxos};
    ...
    \draw [dashed] (Sn) -- (hn);
  \end{tikzpicture}
  \caption{A Paxos--enabled switch $S_1$ mitigates the need for special code on the servers.}
  \label{figure:paxos.on.switch}
\end{figure}

In figure \ref{figure:paxos.on.switch}, the switches themselves enable
