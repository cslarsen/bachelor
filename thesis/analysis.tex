...
\section{Baseline --- ICMP ping \acs{RTT} on L2 learning switch}
...
configurations, because all other simulations use it for forwarding packets.
...
\begin{figure}
  ...
  \caption{Baseline topology with three switches $S$ and their controllers
    $C$.  The client $c_0$ will send ICMP ping packets to the farthest node
      $h_9$.  The packets will go through four links with a configured
...
assume that $L_{C,S}\to\ms{0}$ when the flow tables have been ramped up.
...
We could somehow attempt to measure $K$, but we will simply set it to zero
as well.

The link latencies from figure
\ref{figure:graph.three.switches}
\begin{gather}
  2\sum_n^4 L_n = 2\cdot4\cdot\ms{5} = \ms{40}~\text{where}~L_1 = L_{h_1,S_1},~\dots
\end{gather}
along with the above simplifications give
\begin{gather}
...
most of the processing move over to the controllers. The results for
...
\begin{figure}[H]
  ...
  \caption{\acs{RTT}s for baseline test.  Medians are shown in red and means in gray.
    These plots do not show the large \acs{RTT}s during ramp--up.
    The lower \acs{RTT} plots are for ICMP ping running with flow table
    entries, and the larger without.}
